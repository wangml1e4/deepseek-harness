...
// + wires the panel and alternate-surface actions through the inject hook; teardown cascades
...
describe('ui-layout client apply', () => {
  ...
  it('provides ctx.layout and registers AppFrame into root with the six child declarations', async () => {
    ...
    expect(slots.spec('shell.center')).toEqual({ kind: 'chain', scope: 'root' })
    expect(slots.spec('shell.details')).toEqual({ kind: 'chain', scope: 'root' })
    expect(slots.spec('shell.overlay')).toEqual({ kind: 'list', scope: 'root' })
  })

  it('injects the surface snapshot and attaches the layout actions', async () => {
    const { ctx, slots } = await bench()
    const fiber = ctx.plugin({ inject: [...inject], apply })
    await fiber.await()
    const actions = {
    ...
    expect(injected).toEqual({ hooks: { surface: layout } })
    layout.toggleSidebar()
    expect(actions.toggleSidebar).toHaveBeenCalledOnce()
    layout.openSurface({ id: 'taskboard', context: 'workspace-1' })
    expect(actions.closeDetails).toHaveBeenCalledOnce()
    expect(layout.getSnapshot()).toEqual({ id: 'taskboard', context: 'workspace-1' })
  })
  ...
  it('teardown unwinds the service, the root registration, and the child declarations', async () => {
    ...
    expect(slots.spec('shell.center')).toBeUndefined()
