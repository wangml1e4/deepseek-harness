...
The details surface edits title, Markdown description, status, priority, assignee, labels, and day-granularity dates. It lists append-only Comments and Activity, manages directed dependency relations, and labels each blocking predecessor as waiting for predecessor completion or waiting for code integration. It archives an Issue without exposing permanent deletion. Attachments accept every file type up to 25 MB; the surface lists metadata, previews images, downloads bytes through the controlled Host Remote, and calls permanent attachment deletion only after a browser confirmation. Returning an `in_review`, `blocked`, or `done` Issue to `todo` reveals the required reason field before submission.

The Taskboard header opens Patrol settings in the existing right details column. Its switch starts disabled; the panel saves one of the fixed `5m`, `30m`, `1h`, `2h`, `6h`, `12h`, or `24h` intervals plus Base Branch, Agent Preset, provider, model, reasoning effort, and Permission Preset choices. It shows the next due time, active state, permanent Run/Attempt history, aggregated token usage, structured Provider diagnostics, and a manual `Run now` action. An eligible `todo` Issue also has a scoped run action. Issue details link to the bound Session and display its branch, Base Branch, worktree path and presence, result commit, bounded committed diff, and independent Reviewer findings. A confirmed worktree-removal action remains visible only while the physical directory exists; the Host enforces cleanliness and Base Branch integration, and the details retain the Session, branch, diff, and review evidence afterward. Human review moves `in_review` to `done` or returns it to `todo` with a required reason; it never merges code.
