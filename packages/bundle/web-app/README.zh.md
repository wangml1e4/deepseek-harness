...
本组合包还组装 Workspace Taskboard Host 路径：Taskboard Service Definition、SQLite Provider、Typert Remote 和内置 `manage-taskboard` skill。Remote 可供浏览器 API 外观和单独安装的 `taskctl` 二进制使用；Taskboard Web UI 会在后续 Client 层加入。
