...
The bundle also assembles the Workspace Taskboard Host path: the Taskboard Service Definition, SQLite Provider, local Patrol execution support, Typert Remote, and bundled `manage-taskboard` skill. Patrol execution support owns only local Git worktrees and persistent Agent acquisition until the coordinator hands it a durable claim; it performs no Git network or integration operation. The Remote is available to the browser API facade and the separately installed `taskctl` binary.
...
- **Taskboard is Host-only in this layer** — its durable API and interactive skill are mounted, but the board, list, dashboard, Gantt, and Patrol views are not registered yet.
