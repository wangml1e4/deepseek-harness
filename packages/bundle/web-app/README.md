...
The bundle also assembles the Workspace Taskboard Host path: the Taskboard Service Definition, SQLite Provider, Typert Remote, and bundled `manage-taskboard` skill. The Remote is available to the browser API facade and the separately installed `taskctl` binary; the Taskboard Web UI arrives in the later Client layer.
