...
The plugin also declares its route in the configurable-provider directory (`ctx.llm.listConfigurableProviders()`): provider `deepseek-official`, settings namespace `llm-deepseek`, empty settings path because the whole section is the profile, and activation path `enabled`. Configuration surfaces use that entry to offer this adapter alongside dormant pi-ai providers and to switch the route without guessing its config layout.
