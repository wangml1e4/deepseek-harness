...
ProjectionDefinition<'tokenUsage', TokenUsageState> = {
  ...
  init: () => ({ totals: zeroUsageBuckets(), last: null }),
  apply: (state, event) => {
    let turn: number
    let step: number
    ...
    const buckets = usageBucketsFrom(usage)
    const previous = state.last !== null
      && state.last.turn === turn
      && state.last.step === step
      ? state.last.buckets
      : undefined
    if (previous !== undefined && usageBucketsEqual(previous, buckets)) return state

    return {
      totals: replaceUsageBuckets(state.totals, previous, buckets),
