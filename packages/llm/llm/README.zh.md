...
- `ctx.llm.listConfigurableProviders(): LlmConfigurableProvider[]` 按声明顺序列出可配置提供方目录；配置界面将其与 `listProviders()` 合并，为每个条目标注存活或休眠。条目可携带 `enabledPath`，即配置界面为启用或停用该路由所写入的确切布尔设置路径；还可携带 `declared`，表示拥有该路由的适配器是否只因配置点名才知道它。两项适配器特有事实都只能由适配器回答：缺少 `enabledPath` 表示不提供手动启停控件；缺少 `declared` 只表示该适配器不区分两种来源，不能据此判断路由是否随产品交付。
