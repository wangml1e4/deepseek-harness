...
- 它只覆盖交互式 Taskboard 工作。后续由 Host 持有的 Patrol 调度器会直接针对 Taskboard Service 完成原子资格判定、认领、Session 绑定和生命周期回写。
- 附件、Patrol 控件、Git 与 Session 证据、独立 Reviewer 结果以及人工审查操作会在后续 stack 层加入。
