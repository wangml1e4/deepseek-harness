...
- Each terminal Attempt retains Provider-reported token usage and the latest structured Provider error observed during its execution. Run completion aggregates usage across its Attempts and retains the latest Provider error; Git, permission, and review failures remain ordinary error text rather than Provider errors.
