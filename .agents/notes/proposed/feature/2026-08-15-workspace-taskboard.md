...
The first six stack layers provide the Workspace-owned Taskboard Service Definition, local SQLite Provider, Host Typert Remote, JSON `taskctl` command line, bundled `manage-taskboard` skill, browser Dashboard, Board, List, Gantt and Issue details, durable Patrol scheduling and claim state, and local Git, Session, and permission execution support. The standard Web Host mounts the interactive roles together, and the installed `dsh` package exposes both `dsh` and `taskctl` executables. The Remote and CLI cover Workspace metadata, Issue lifecycle and order, Comments, Activity, and dependency relations. Successful writes publish a contained `taskboard/changed` event so the active browser projection refreshes without polling.

The browser UI enters from each Workspace row, replaces the conversation center with the selected Taskboard, and reuses the existing right details column. Its view, Gantt scale, and filter preferences persist in browser storage, while authoritative Issue data remains Host-owned. The Gantt renderer keeps unscheduled Issues in its grid, draws canonical `blocks` links between scheduled bars, and persists one dragged Issue's dates without cascading changes.

Every ensured Taskboard now owns a default-off `1h` Patrol Policy with execution choices. Policy saves calculate a new due time, overdue scheduled triggers consume one missed cadence point, scheduled overlaps persist `skipped_global_busy`, manual overlaps reject, a database uniqueness rule reserves one active Run across the Host, and terminal Run history is permanent. Atomic claims create permanent ordered Attempts, and only a permission-blocked Attempt permits another claim in that Run. A permanent Development Context fixes each Patrol-owned Issue's local branch, worktree, exact Session, Agent Preset, model selection, and Permission Preset. The mounted execution Consumer uses a restricted local Git vocabulary, resumes only that Session after it has started, and rejects unattended tool approvals without delegating them. It remains side-effect-free until the next layer installs the Host timer, scanner, prompt, and independent Reviewer; human review controls and recovery also remain proposed.
...
- Host restart recovers an unfinished Run before new triggers, attempts the exact Session and worktree once, and moves the Issue to `blocked` without replacement when recovery is impossible.
