...
Version one has no provider-quota-aware pause because Harness providers expose no common quota query. Patrol Run history retains token usage and Provider errors. Canonical Session usage events and terminal model-stream failures are the accounting source: each Attempt accumulates implementation, Reviewer, correction, and recovery turns, while Run completion aggregates its Attempts and keeps the latest structured Provider failure. Recovery rebuilds pre-crash accounting from the exact persisted implementation Session and any recorded Reviewer Session before it appends resumed turns. Git, permission, and review failures remain separate error text so Patrol does not infer Provider failures from messages. A quota or rate-limit failure records its reason, moves the current Issue to `blocked`, and ends the Run without disabling the fixed-interval policy; later scheduled Runs may process other eligible Issues.
