...
The first seven stack layers provide the Workspace-owned Taskboard Service Definition, local SQLite Provider, Host Typert Remote, JSON `taskctl` command line, bundled `manage-taskboard` skill, browser Dashboard, Board, List, Gantt and Issue details, durable Patrol state, local Git and exact Session execution, fixed-interval coordination, an independent Reviewer, and human review controls. The standard Web Host mounts the interactive roles together, and the installed `dsh` package exposes both `dsh` and `taskctl` executables. The Remote and CLI cover Workspace metadata, Issue lifecycle and order, Comments, Activity, dependencies, Patrol configuration and history, manual Runs, and Issue evidence. Successful writes publish a contained `taskboard/changed` event so the active browser projection refreshes without polling.
