...
Every ensured Taskboard now owns a default-off `1h` Patrol Policy. Policy saves calculate a new due time, overdue scheduled triggers consume one missed cadence point, scheduled overlaps persist `skipped_global_busy`, manual overlaps reject, a database uniqueness rule reserves one active Run across the Host, and terminal Run history is permanent. These scheduling operations are not exposed through Remote or UI and do not start Agent work until the next execution layer installs the Host timer and Agent Consumer. Attachments, Git and Session execution, independent Reviewer, human review controls, and recovery remain proposed.
