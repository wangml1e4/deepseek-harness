...
**llm seam 声明可配置性并公布拓扑。**`registerConfigurableProviders()` 是一个全有或全无、以 fiber 为作用域的目录，条目为 `{provider, displayName, settingsNs, settingsPath, enabledPath?}`——前者是配置页寻址一条可能尚不存在路由的 profile 地址，后者是适配器提供手动启停时允许界面写入的确切布尔设置。整分节提供方与 profile 字典把开关存放在不同深度，所以路径必须由适配器声明，界面不猜字段名。`listConfigurableProviders()` 在 wire 处理器里与存活路由合并，未声明的存活路由因此仍报告为激活。零负载的 `'llm/adapters-updated'` 事件从全部四个注册／注销提交点触发，listener 派发带异常隔离（INVARIANT 重抛），沿用 settings/commands 的先例。`llm-deepseek` 的路由重命名为 `deepseek-official`，因为 pi-ai catalog 名正言顺地拥有 `deepseek` 这个聚合器条目；依预发布立场，不设别名。
...
- **从 `settingsPath` 推断 `enabled` 字段**——当前适配器恰好使用这个拼写，但配置界面并不拥有适配器 profile 的布局。显式 `enabledPath` 让不提供手动启停的适配器维持原状，也让未来适配器可以把布尔值放在别处，而无需引入 UI 约定或兼容垫片。
- **由 `models` 桥接插件持有提供方配置**——与请求级 seam note 相同的否决理由：按插件划分的 namespace 加上目录声明已经给了 UI 需要的一切；桥接层的统一字典会把适配器映射那层间接重新引进来。
